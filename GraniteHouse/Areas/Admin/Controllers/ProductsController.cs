...
namespace GraniteHouse.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        {
            var products = _db.Products.Include(m => m.ProductTypes).Include(m => m.SpecialTags).OrderBy(m => m.Name);
        ...
        {
            ProductsVM.Products.SpecialTagsID = 6;
            return View(ProductsVM); // productsVM needed to display pulldowns for product types and special tags.
        }
        ...
        public async Task<IActionResult> CreatePOST()
        {
            ...
            if (files.Count != 0)
            {
                //Image has been uploaded, product needs to be saved so we have the product.ID
            ...
            {
                //when user does not upload image.  Copy the default_img.png to Product.Id + ".png"
        ...
        public async Task<IActionResult> Edit(int id)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    //if user uploads a new image, need to delete the old image file (since the product name has the ID as the filename).
                    // hmmm, no way to just delete an image and leave blank.
                    var uploads = Path.Combine(webRootPath, SD.ImageFolder);
